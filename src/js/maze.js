...
}
{
  ...
  mazeHelpers.builder = (function () {
    ...
    var getCandidates = function (cell) {
      ...
      // Check North
      if (cell.row > 0) {
        if (board[cell.row - 1][cell.column].isVisited == false) candidates.push(board[cell.row - 1][cell.column]);
      }
      ...
      // Check East
      if (cell.column > 0) {
        if (board[cell.row][cell.column - 1].isVisited == false) candidates.push(board[cell.row][cell.column - 1]);
      }
  ...
  function toggleAutoMode() {
    ...
    if (
      !isAutoMode &&
      (!$BrowserInfo.IsBrowserInternetExplorer() ||
        ($BrowserInfo.IsBrowserInternetExplorer() && $BrowserInfo.IsBrowserInternetExplorer9RCPlusOrLater()))
    ) {
  ...
  function scaleAndCenterMaze(maze) {
    if ($BrowserInfo.IsBrowserInternetExplorer() && !$BrowserInfo.IsBrowserInternetExplorer9RCPlusOrLater()) {
      ...
      var containerWidth = window
        .getComputedStyle(document.getElementById("MazeContainer"), null)
      ...
      if (mazeHelpers.builder.getSize() == baseSize) {
        ...
        maze.style.msTransform =
          "translate(" + translation + ", 0px) scale(" + baseSize / mazeHelpers.builder.getSize() + ")";
        maze.style.webkitTransformOrigin = "top left";
        maze.style.webkitTransform =
          "translate(" + translation + ", 0px) scale(" + baseSize / mazeHelpers.builder.getSize() + ")";
        maze.style.MozTransformOrigin = "top left";
        maze.style.MozTransform =
          "translate(" + translation + ", 0px) scale(" + baseSize / mazeHelpers.builder.getSize() + ")";
        maze.style.OTransformOrigin = "top left";
        maze.style.OTransform =
          "translate(" + translation + ", 0px) scale(" + baseSize / mazeHelpers.builder.getSize() + ")";
      }
    }
  }
